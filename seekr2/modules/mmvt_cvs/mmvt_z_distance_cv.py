"""
Structures, methods, and functions for handling Z-distance CVs.
(CV that depends only on the difference of z coordinates of two 
groups) - usable for membrane/binding or membrane/permeant systems.
"""
...
class MMVT_z_distance_CV(MMVT_collective_variable):
    """
    A z-distance collective variable represents the difference in z
    coordinate between two different groups of atoms.
    ...
    def __init__(self, index, groups):
        ...
        self.name = "mmvt_z_distance"
        self.openmm_expression = "step(k*(z2 - z1 - value))"
        self.restraining_expression = "0.5*k*(z2 - z1 - value)"
        self.cv_expression = "z2 - z1"
        self.num_groups = 2
        self.per_dof_variables = ["k", "value"]
        self.global_variables = []
        self._mygroup_list = None
        self.variable_name = "z"
        return

    def __name__(self):
        return "MMVT_z_distance_CV"
    ...
    def make_namd_colvar_string(self):
        ...
        MMVT bounces.
        """
        raise Exception("MMVT z-distance CVs are not available in NAMD")
    ...
    def get_openmm_context_cv_value(self, context, positions=None, system=None):
        ...
        value = com2[2] - com1[2]
        return value.value_in_unit(unit.nanometers)
    ...
    def check_value_within_boundary(self, value, milestone_variables, 
        ...
        if milestone_k*(value - milestone_value) > tolerance:
            if verbose:
                warnstr = """The z-coords of masses of atom group1 and atom
...
boundary at {:.4f} nm.""".format(value, milestone_value)
    ...
    def check_mdtraj_close_to_boundary(self, traj, milestone_variables, 
        ...
        if abs(avg_distance) > max_avg or std_distance > max_std:
            if verbose:
                warnstr = """The z-coords between the system and central 
...
def make_mmvt_z_distance_cv_object(z_distance_cv_input, index):
    """
    Create a SphericalCV object to be placed into the Model.
    """
    group1 = base.parse_xml_list(z_distance_cv_input.group1)
    group2 = base.parse_xml_list(z_distance_cv_input.group2)
    groups = [group1, group2]
    cv = MMVT_z_distance_CV(index, groups)
    return cv