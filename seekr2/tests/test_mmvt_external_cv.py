...
def test_mmvt_external_boundary(tmp_path):
    ...
    my_cv = mmvt_external_cv.MMVT_external_CV(index=0, groups=[[0], [1]])
    my_cv.cv_expression = "x1"
    my_cv.openmm_expression = "step(k*(x1 - value))"
    my_cv.restraining_expression = "0.5*k*(x1 - value)^2"
    ...
    pdb_filename1 = os.path.join(tmp_path, "external_mmvt_toy1.pdb")
    ...
    pdb_filename2 = os.path.join(tmp_path, "external_mmvt_toy2.pdb")
...
def test_mmvt_spherical_restraint(tmp_path):
    # Set initial variables
    milestone_variables1 = {"k":9000.0, "value":0.5}
    milestone_variables2 = {"k":9000.0, "value":0.4}
    my_variables1 = [1.0, 9000.0, 0.5]
    my_variables2 = [1.0, 9000.0, 0.4]
    positions1 = [[0.5, 0.0, 0.0], [0.5, 0.4, 0.0]]
    positions2 = [[0.4, 0.0, 0.0], [0.4, 0.4, 0.0]]
    ...
    toy_simulation = create_toy_system.make_toy_simulation_and_context(
        toy_system, toy_topology, positions1)
    trajectory_filename1 = os.path.join(tmp_path, "external_mmvt_toy1.dcd")
    pdb_filename1 = os.path.join(tmp_path, "external_mmvt_toy1.pdb")
    common_sim_openmm.write_toy_pdb_file(toy_topology, positions1, pdb_filename1)
    ...
    assert np.isclose(value_context, 0.5)
    ...
    toy_simulation.context.reinitialize(preserveState=True)
    toy_simulation.context.setPositions(positions2)
    trajectory_filename2 = os.path.join(tmp_path, "external_mmvt_toy2.dcd")
    pdb_filename2 = os.path.join(tmp_path, "external_mmvt_toy2.pdb")
    common_sim_openmm.write_toy_pdb_file(toy_topology, positions2, pdb_filename2)
    ...
    pdbtraj2 = mdtraj.load(pdb_filename2)
    value = my_cv.get_mdtraj_cv_value(pdbtraj2, frame_index=0)
...
def test_mmvt_spherical_voronoi_boundary():
    ...
    me_val = 0.5
    neighbor_val = 0.7
    positions1 = [[0.5, 0.0, 0.0], [0.3, 0.4, 0.0]]
    ...
    positions2 = [[0.7, 0.0, 0.0], [0.0, 0.7, 0.0]]
    toy_simulation.context.setPositions(positions2)
    toy_state_me = toy_simulation.context.getState(getEnergy=True, groups={1})
    toy_state_neighbor = toy_simulation.context.getState(getEnergy=True, groups={2})
    boundary_val_me = toy_state_me.getPotentialEnergy()
